...
argument-hint: "[--background|--wait] [--resume|--fresh] [--model <model|sol|terra|luna>] [--effort <none|minimal|low|medium|high|xhigh|max>] [what Codex should investigate, solve, or continue]"
...
- Leave the model unset unless the user explicitly asks for one. Normalize `sol`, `terra`, and `luna` to the corresponding GPT-5.6 model IDs.
